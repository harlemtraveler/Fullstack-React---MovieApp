...
if (module.hot) {	// check to see if the Hot Loader is enabled by checking if the “module.hot" object is defined
    module.hot.accept('./routes', ( ) => {	// we accept any changes using the “.accept” module. Her it’s any changes from the “App” file…”src/App.js"
	   // accept hot change…when any change occurs…manually doing something
       const NextApp = require('./routes').default	// re-require the “App.js” file and render a new instance in the component
       renderApp(<Routes />)
    })
} // when any changes occur in our application files, we want to reload the application using the “renderApp" function

renderApp(<Routes />) // calls the render app function directly, tells Hot Loader what to do happen when to certain conditions are met
